// Removes first occurence of an element by Value and returns its index.
...
const ROW = 12, COL = 10,
		sound = {
			...
			moved: new Audio('../sound/takeAhead.wav'),
...
$(document).ready( () => {
	...
	for ( let i = 0, select = []; i < 16; i++ ) {
		select = ( i > 7 ? [`:nth-child(${i - 5}) td`, `rank-${16 - i}`] :
			[` td:nth-child(${i + 2})`, `file-${String.fromCharCode(i + 97)}`] );
		$(`#board tr${select[0]}:not(.invalid)`).addClass(select[1]);
	}
...
//		$('#btn-menu *').click( () => $('#btn-menu').toggle() );
		...
		game.variant = $('#game-type').val();
		setPiecePosition();
		countDown();
	});
//	setTimeout( () => $('#start-game #start-btn').click(), 1500 );
	newGame();
} );

function newGame() {
	sqr.not('.invalid').add('.captured').empty();
	sqr.not('.invalid').removeClass('path capture castle');
	PGN.html('<span></span>');
	...
	piece = {
		ch: '', select: false, 
		trail: [0, 0], info: [[], []], path: [],
		pos: [[25, 21, 22, 23, 24, 26, 27, 28, 31, 32, 33, 34, 35, 36, 37, 38],[95, 91, 92, 93, 94, 96, 97, 98, 81, 82, 83, 84, 85, 86, 87, 88]]
//		pos: [[],[]]
	};
	game = {
		moves: 0, isCustomSetup: false, turn: true,		// true for White turn
		last: [false, -1], history: [piece.info],
		switchTime: undefined, variant: '',
		captured: [['', '', '', '', ''], ['', '', '', '', '']],
...
function setPiecePosition() {
	let piecePosition = [ [14,16,15,13,12,15,16,14], [14,16,15,13,12,15,16,14] ], 
		isUpsideDown = false,
		getRandomPosition = () => {
	...
	switch ( game.variant ) {
		case '960': piecePosition[0] = piecePosition[1] = getRandomPosition();
			break;
		case 'transcendental':
			piecePosition = [ getRandomPosition(), getRandomPosition()];
			break;
		case 'upside-down': piece.pos.reverse();
			isUpsideDown = true;
			game.canCastle = [[false, false, false], [false, false, false]];
			break;
		case 'pawn-game': piecePosition = [[12],[12]];
			game.canCastle = [[false, false, false], [false, false, false]];
			piece.pos = piece.pos.map( p => p.splice(-8) );
			break;
		case 'horde': piecePosition.splice(1);
			piece.pos[1] = [...piece.pos[1], ...piece.pos[1].map(n => n - 20), 52, 53, 56, 57];
			$('.rank-1, .rank-2, .rank-3, .rank-4, .file-b.rank-5, .file-c.rank-5, .file-f.rank-5, .file-g.rank-5').html('&#9817;');
			break;
		case 'king-of-hill':
			$('.file-d.rank-5, .file-e.rank-5, .file-d.rank-4, .file-e.rank-4').addClass('king-of-hill');
			break;
		case 'custom': piece.pos = [[],[]];
			customSetup();
			return;
	}
	
	piecePosition.forEach( (pos, i) => {
		let a = 0, iOf0 = pos.indexOf(12) + 1;
		pos.forEach( (n, j) => {
			const thisSqr = j + COL * (i * 7 + 2) + 1;
			sqr.eq(thisSqr).html(`&#98${(!i ^ isUpsideDown) * 6 + n};`);
//			piece.pos[i].push(thisSqr, j + COL * (i * 5 + 3) + 1);

			if ( n == 14 ) game.canCastle[i][a++ * 2] = i * 70 + j + 21;
		});

		$(`.rank-${7 - 5 * i}`).html(`&#98${(!i ^ isUpsideDown) * 6 + 17};`);
		//		king position first
	...
	/*for (let i = 0, a, iOf0 = piecePosition.indexOf(12) + 1; i < 2; i++) {
		a = 0;
		piecePosition[i].forEach( (n, j) => {
			const thisSqr = j + COL * (i * 7 + 2) + 1;
			sqr.eq(thisSqr).html(`&#98${!i * 6 + n};`);
			piece.pos[i].push(thisSqr, j + COL * (i * 5 + 3) + 1);
...
//		king position first
		piece.pos[i].unshift( piece.pos[i].splice(
			piece.pos[i].indexOf(iOf0 + COL * (i * 7 + 2)), 1)[0] );
		$(`.rank-${7 - 5 * i}`).html(`&#98${!i * 6 + 17};`);
	}*/
}

function customSetup( on_off = true ) {
	game.isCustomSetup = on_off;
	
	if ( on_off == 'clear-board' ) {
		if ( sqr.not('.invalid').text() ) {
			sqr.not('.invalid').text('');
			piece.pos = [[],[]];
		}
		else {
			for (let i = 0; i < 2; i++) {
				$(`.rank-${7 - 5 * i}`).html(`&#98${!i * 6 + 17};`);
				[14,16,15,13,12,15,16,14].forEach( (n, j) => {
					const thisSqr = j + COL * (i * 7 + 2) + 1;
					sqr.eq(thisSqr).html(`&#98${!i * 6 + n};`);
					piece.pos[i].push(thisSqr, j + COL * (i * 5 + 3) + 1);
				});
	...
	const customs = $('#board tr:nth-last-child(-n+2) td:not(:first-child):not(:last-child), .contain-3').add(PGN);
	
	if ( game.isCustomSetup ) {
		customs.addClass('custom');
		
		sqr.eq(108).html("<button onclick=\"customSetup('clear-board')\">&#10008;</button>");
		sqr.eq(118).html("<button onclick='customSetup(false)'>&#10004;</button>");

		for ( let i = 0; i < 12; i++ ) {
			sqr.eq(i + 101 + (i > 5) * 4).html(`&#98${i + 12};`);
		}

		let lastThis;
		$('td.custom').not(':nth-last-child(-n+3)').click( function () {
			if ( lastThis && this === lastThis.get(0) ) return;
			const $this = $(this);
			piece.select = $this.text();
			$this.add(lastThis).toggleClass('castle');

			lastThis = $this;
			game.turn = sqr.index( $this ) < 111;
//			console.log(game.turn );
		} );
	}
	else {
		game.turn = true;
		const customTD = $('td.custom');
		customTD.text('');
		customTD.off();
		customs.removeClass('custom');
	}
...
function clicked() {
	const thisSqr = sqr.index( this );
	if ( game.isCustomSetup ) {
		const $this = $(this);
		if ( $this.text() ) {
			$this.text( '' );
			if ( piece.pos[0].remove( thisSqr ) == -1 ) piece.pos[1].remove( thisSqr );
		}
		else if ( !piece.select ) console.warn('Select a Piece First!');
		else {
			$this.text( piece.select );
			piece.pos[+game.turn].push( thisSqr );
			sound.takeBack.play();
		}
		
	}
	else if ( piece.select ) {								// Second Click
...
function processOn( c1, c2 ) {
	...
	if ( game.variant == 'king-of-hill' &&  piece.ch == 'K' && sqr.eq(c2).hasClass('king-of-hill') ) {
		gameOver ('Hill Rule');
	}
...
function movePiece( info, flag = 0 ) {
	...
	if ( current.eq(game.moves + 1 - !toTakeBack).text().substr(-1) == 'O' ) {
		...
		if ( game.variant == 'atomic' ) {
			...
			if ( toTakeBack ) {
				...
				MAGIC_ARR.K.map(n => n + info[0][0]).forEach( (n, i) => {
					if ( expl[i+1] != '0' ) {
						sqr.eq(n).text(expl[i+1]);
					}
				});
				info[1][1] = info[1][1][0];
			}
			else {
				MAGIC_ARR.K.map(n => n + info[0][1]).forEach( n => {
					if ( game.alphCode(n) == 'P' ) {
						expl += '0';
					}
					else {
						expl += sqr.eq(n).text();
						sqr.eq(n).text('');
						if ( piece.pos[0].remove(n) == -1 ) piece.pos[1].remove(n);
					}
				});
				sqr.eq(info[1][0]).text('');
				setTimeout( () => piece.pos[bl].remove(info[0][1]), 10);
				info[1][1] = expl;
	...
	game.turn = !game.turn;
	if ( !flag ) {
		isKing.inCheck = checkCheck();
		if ( isKing.inCheck ) {
			let isCheckmate = compMove( true );
			console.log ( isCheckmate );
			if ( isCheckmate ) {
				gameOver( 'Checkmate' );
			}
			current.eq(game.moves).append( isCheckmate ? '#' : '+' );
		}
	}
	else if ( isKing.inCheck ) isKing.inCheck = false;

	if ( toTakeBack ) toPlay = sound.takeBack;
	else if ( '+#'.includes(current.eq(game.moves).text().slice(-1)) )
		toPlay = sound.check;
	else if ( flag == 1 ) toPlay = sound.moved;
...
function compMove( task = false ) {
	...
	if ( task ) return possibleMoves == 0;
	else setTimeout( () => {
		showTrail( toMove );
...
function gameOver( reason ) {
	$('#game-over p').text(`${game.turn ? 'Black' : 'White'} has Won the Game by ` + reason);
	sqr.off('click');
	setTimeout ( () => {
		new Audio('../sound/gameOver.wav').play();
		$('#game-over').toggle(200);
